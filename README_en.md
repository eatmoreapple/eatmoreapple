<!-- Typing SVG -->
<h1 align="center">Hi there, I'm eatmoreapple! 👋</h1>

<p align="center">
  <a href="mailto:eatmoreorange@gmail.com">
    <img src="https://img.shields.io/badge/-Email-FF6B6B?style=flat-square&logo=Gmail&logoColor=white" alt="Email">
  </a>
  <a href="https://github.com/eatmoreapple">
    <img src="https://img.shields.io/badge/-GitHub-181717?style=flat-square&logo=github" alt="GitHub">
  </a>
  <a href="https://wechat.com">
    <img src="https://img.shields.io/badge/-WeChat-07C160?style=flat-square&logo=wechat&logoColor=white" alt="WeChat">
  </a>
</p>

## About Me ✨

A developer passionate about technology, currently based in Hangzhou, focused on Golang and Python, and enjoys solving problems through code. In spare time, I actively participate in open-source projects and grow with the community.

📖 **[中文版 README (Chinese Version)](README.md)**

## 🌟 Open Source Projects

### 📱 openwechat - Golang WeChat SDK

<p align="center">
  <img src="https://img.shields.io/github/stars/eatmoreapple/openwechat?style=social" />
  <img src="https://img.shields.io/github/forks/eatmoreapple/openwechat?style=social" />
</p>

A powerful WeChat personal account robot SDK based on web WeChat protocol. With it, you can easily implement WeChat message monitoring, auto-reply, friend management and other functions.

- 🔧 **Out of the box** - Just a few lines of code to start the WeChat robot
- 🔄 **Event driven** - Support multiple message types and callback functions
- 🛡️ **Safe and stable** - Simulate real client behavior to avoid restrictions
- 📚 **Complete documentation** - Provide detailed usage guides and sample code

🔗 [Repository](https://github.com/eatmoreapple/openwechat) | 📖 [Documentation](https://openwechat.readthedocs.io/zh/latest/)

### 🍹 juice - Golang SQL Mapper Framework

<p align="center">
  <img src="https://img.shields.io/github/stars/eatmoreapple/juice?style=social" />
  <img src="https://img.shields.io/github/forks/eatmoreapple/juice?style=social" />
</p>

A Golang SQL mapping framework inspired by MyBatis, making database operations simpler and more intuitive. Supports advanced features such as dynamic SQL and result mapping.

- ⚡ **High performance** - Zero reflection design, extremely high operating efficiency
- 🎯 **Easy to integrate** - Seamless docking with standard libraries, low learning cost
- 🌈 **Highly flexible** - Support complex dynamic SQL construction
- 📦 **Lightweight** - No third-party dependencies, simple deployment

🔗 [Repository](https://github.com/eatmoreapple/juice) | 📖 [Documentation](https://juice-doc.readthedocs.io/en/latest/index.html)

---

## GitHub Stats 📊

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=eatmoreapple&show_icons=true&theme=dracula" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=eatmoreapple&layout=compact&theme=dracula" />
</p>

---

## 📫 Let's Connect!

I'm always interested in connecting with fellow developers and exploring new opportunities! I'm currently considering new job opportunities, so if you have any suitable opportunities or project collaborations, please feel free to contact me.

<p align="center">
  <a href="mailto:eatmoreorange@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail">
  </a>
  <a href="./img/wechat.jpg">
    <img src="https://img.shields.io/badge/WeChat-07C160?style=for-the-badge&logo=wechat&logoColor=white" alt="WeChat">
  </a>
</p>

## ☕ Support Me

If you like my work, please consider supporting me by buying me a coffee.

<p align="center">
  <a href="./img/wechat_pay.jpg">
    <img src="https://cdn.buymeacoffee.com/buttons/v2/default-yellow.png" alt="Buy Me A Coffee" width="150">
  </a>
</p>

---
<p align="center">
  Made with ❤️ by eatmoreapple | Let code have more warmth
</p>









